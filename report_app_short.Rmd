...
# group <- "District" # Which variable to break down by. Usually ethnicity.
# cat_of_interest <- "Prefer not to say / Unknown"
# school_number <- 40
...
if(group == "District"){
   ...
   data <- data %>%
     dplyr::mutate(breakdown = ifelse(breakdown == "Prefer not to say / Unknown", "Prefer not to say/Unknown", breakdown))
   
  rename_group = "District"
  
if(cat_of_interest == "Prefer not to say / Unknown") {
  cat_of_interest <- "Prefer not to say/Unknown"
}
...
`r create_sum_sentence_cat_interest()`
...
```{r sexuality_graph}
create_basic_plot(df = chk_stats,                    plot_custom_grp = plot_custom_grp,                    plot_title = plot_title, rotate = 25)
```
...
plot_title = "Prefered type of support for mental health"
