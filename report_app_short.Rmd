...
# group <- "bullied" # Which variable to break down by. Usually ethnicity.
# cat_of_interest <- "Bullied"
# school_number <- 40
# data <- data$data$bullied
# year_report <- 2022
    ...
    if (is.na(total_resp)) { 
      ...
      if (!is.na(group_of_interest)[1]) {#& chk_var != group
        ...
        add <- paste0(" Among them, ", paste0(
          prop, " were ", grp_df$breakdown, collapse = ", "), " respondents. ")
        ...
        sentence <- paste(sentence, "<br><br>", paste0("The most common response for ", group_of_interest, " respondents was '", most_common$most_common, 
                                                       "', which made up ", most_common$most_v, " of responses and the least common response for ", 
                                                       group_of_interest, " respondents was '", least_common$least_common, "', with ", least_common$least_v, " of responses.", 
                                                       collapse = "<br><br>"))
    ...
    if (is.na(total_resp)) { 
      ...
      if(!is.na(group_of_interest)) {
        ...
        for(group in 1:length(c("All Responses", group_of_interest))) {
          ...
          if (binary) {
            temp <- paste0("Out of responses from ", group_name, " respondents, ", 
                           glue::glue_collapse(glue::glue("{df$value} selected '{df$question_text}'"), ", ", last = ", and "))
          } else{
            temp <- paste0("The number of ", group_name, " respondents who stated '", df$response[1], "' was ",
...
**This version of the report is grouped by "`r rename_group`" the category of interest is "`r cat_of_interest`"**. 
...
`r 
if(group != "sexuality"){
 create_sum_sentence_herts()
 } else {
create_sum_sentence_cat_interest()}`
...
chk_var <- c('condition_physical', 'condition_mental', 'condition_disability',
             'condition_send', "condition_autism", "condition_adhd")
...
`r if(group != "caring"){create_sum_sentence_herts()} else {create_sum_sentence_cat_interest()}`
...
**"Are you currently or have you ever been a child looked after (e.g., lived with foster carers/lived in a care home) or are you adopted?"**
...
``` {r diet_summary}
chk_var <- c('diet_water','diet_fastfood','diet_fizzydrinks','diet_energydrinks','diet_sweets', 'diet_breakfast')
chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)
...
``` {r smoke_secondhand_summary}
chk_var <- c('smoke_secondhand')
chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)
...
```{r smoke_secondhand_graph}
create_basic_plot(df = chk_stats,                    plot_custom_grp = plot_custom_grp,                    plot_title = plot_title)
```

### Table
```{r smoke_secondhand_table}
create_tbl(chk_diff)
```
...
chk_var <- c('amphetamines_exp', 'cannabis_exp','khat_exp', 'ecstasy_exp','cocaine_exp','crack_exp', 'hallucinogen_exp','heroin_exp','ketamine_exp',
...
top_5 <- tail(sort(filter(chk_stats, breakdown == "All Responses", response == "Yes")$count), 5)
chk_var_brief <- unique(chk_stats$question[chk_stats$count %in% top_5 &
                                       chk_stats$breakdown == "All Responses"])
chk_stats_brief <- dplyr::filter(stats, question %in% chk_var_brief)
```
...
`r create_sum_sentence_herts(dataset = chk_stats_brief, multi = T, value_of_interest = "Yes")[1]`
...
top_2 <- tail(sort(dplyr::filter(chk_stats, breakdown == "All Responses", response == "Yes")$count), 2)
chk_var <- unique(chk_stats$question[chk_stats$count %in% top_2 &
                                       chk_stats$breakdown == "All Responses"])
```
...
“I think the quality of the relationships and sex education I receive at school is good.”;

“If you have sex without a condom, you should get an STI test.”
...
chk_var <- c('sh_pressure','sh_wait','sh_serious','sh_condoms','sh_pregnancy','sh_16', "sh_condomsti",
             "sh_schooled"
)
chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)
...
``` {r wellbeing_summary}
chk_var <- c('life_satisfied', 'life_worthwhile', 'life_happyyesterday')
#***
chk_stats <- dplyr::filter(stats, question %in% chk_var) %>%
...
``` {r Worries_summary}
chk_var <- c('worry_school', "worry_future",
             'worry_bully','worry_phys','worry_mental','worry_family','worry_money','worry_look',
             'worry_socialmedia','worry_relationships','worry_drugsalcohol','worry_environment',
             "worry_racism","worry_friends","worry_incrime","worry_lgbt","worry_violencegirls",
             "worry_treatgirls","worry_violenceboys",
             'worry_crime','worry_gambling','worry_lonely')
...
**"Do you feel hopefull about your future"**
``` {r hopeful_future_summary}
chk_var <- c('hopeful_future')

chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)

plot_title = "Feeling hopeful about their future?"
```
...
```{r hopeful_future_graph}
create_basic_plot(df = chk_stats,                    plot_custom_grp = plot_custom_grp,                    plot_title = plot_title)
```

### Table
```{r hopeful_future_table}
create_tbl(chk_diff)
```
...
chk_var <- c('internet_message','internet_personalinfo','internet_pictures_pressure','internet_met','internet_pictures_upset',"internet_info_provided")
...
chk_var <- c('mental_doctor','mental_school','mental_clinic','mental_youthservice','mental_none','mental_parents','mental_nurse', "mental_charity")
...
## Prefered type of support for mental health {.tabset}
**"If you were struggling with your mental health, what KIND of support would you want? Select up to three" **

``` {r mental_type_summary}
chk_var <- c("mental_type_online", "mental_type_phone", "mental_type_121", "mental_type_group", "mental_type_other")

chk_stats <- filter(stats, question %in% chk_var)

plot_title = "Preferred type of support for mental health"
...
chk_var <- unique(c(chk_var, "mental_school", "mental_nurse"))
...
`r 
if(group != "bullied"){
 create_sum_sentence_herts(multi = T,value_of_interest = "Yes")[1]
 } else {
create_sum_sentence_cat_interest(multi = T)}`
...
## Carrying weapons for protection {.tabset}
...
chk_var <- c('views_listened_school')

chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)

plot_title = "Feels views listened to at school"
```
...
chk_var <- c("clubs_arts",'clubs_perform','clubs_schoolclub','clubs_doe','clubs_music','clubs_religion','clubs_sports','clubs_uniformed','clubs_volunteering','clubs_syp','clubs_other')
...
top_3 <- tail(sort(dplyr::filter(chk_stats, breakdown == "All Responses", response == "Currently attending")$count), 3)
chk_var_brief <- unique(chk_stats$question[chk_stats$count %in% top_3 &
                                       chk_stats$breakdown == "All Responses"])
...
`r create_sum_sentence_herts(dataset = chk_stats_brief, multi = T, value_of_interest = "Currently attending")[1]`
...
## Help and support for prospects {.tabset}
**"Where do you go to get help and support about what to do when you leave school/college such as education, employment, and training?"**

``` {r prospects_support_summary}

chk_var <- c('prospects_support_parents','prospects_support_school','prospects_support_syp', 'prospects_support_jobcentre','prospects_support_friends','prospects_support_online',"prospects_support_idk")

chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)

plot_title = "Where to go for support for prospects"
...
### Graph
```{r prospects_support_graph, fig.height=6.5}
create_multicat_plot_herts(chk_stats, q_coded = q_coded)$result
```


### Table
```{r prospects_support_table}
create_tbl(chk_diff, multi_response = T)
```

## More information about prospects {.tabset}
**"What would you like to have more information on if it was available?"**

``` {r prospects_info_summary}

chk_var <- c('prospects_info_uni','prospects_info_jobs','prospects_info_apprenticeships', 'prospects_info_college','prospects_info_6thform','prospects_info_trainee', "prospects_info_gap",
             "prospects_info_other")


chk_stats <- dplyr::filter(stats, question %in% chk_var)
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var)

plot_title = "What would you like to have more info on?"
...
### Graph
```{r prospects_info_graph, fig.height=6.5}
create_multicat_plot_herts(chk_stats, q_coded = q_coded)$result
```


### Table
```{r prospects_info_table}
create_tbl(chk_diff, multi_response = T)
```
