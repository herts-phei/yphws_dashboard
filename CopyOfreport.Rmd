...
group <- "ethnicity" # Which variable to break down by. Usually ethnicity.
...
create_bullets_herts <- function(df, grouped_by_question = F) {
  ...
  if(nrow(df) == 0){
    ...
    for (i in seq(nrow(df))) {
      ...
      sentences[[i]] <- paste0(sprintf(template, cust_break, current$response, current$value,
                                       current$question_text, current$diff,
                                       current$school.y, current$value.y))
    }
    
    df$sentences <- unlist(sentences)
    
    df <- dplyr::rename(df, "Differences found" = sentences, "Breakdown" = breakdown,
...
This version of the report is grouped by `r group` the catergory of interest is `r cat_of_interest`

This report summarises the results of `r as.character(max(filter(stats) %>% select(denominator)))` pupils from `r length(unique(data$school))` schools who responded to the `r year_report` Young People's Health & Wellbeing Survey (YPHWS).
