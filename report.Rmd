...
board <- pins::board_rsconnect(server = "https://hertscc.managed.mango-solutions.com/connect",                         key = Sys.getenv("CONNECT_API_KEY"))
# Parameters --------------------------------------------------------------

year <- 2021 # year of survey
group <- "ethnicity"#params$var # Which variable to break down by. Usually ethnicity.
cat_of_interest <- "White"#params$cat # Only applicable if group parameter is custom and you'd like to highlight a specific category within the group. Otherwise, state NA
...
# questions coded
# q_coded <- pins::pin_read(board, "hau/YPHWS_question_lookup_2021") %>%
...
# q_coded <- read.csv("data-raw/q_coded.csv") %>%
#       purrr::map_dfr(~ as.character(.)) %>%
#       dplyr::mutate(dplyr::across(where(is.character), ~na_if(., "NA")))
# 
q_coded <- q_coded %>%
  select("question_raw", "question_theme", "question_coded", "question_text", "reworded", "survey_text", "menu_text", "multi_cat", "multi_binary", "question_coded_gen", "survey_text_gen") %>%
  distinct()
# q_coded2$response_of_interest
# survey data
 data2 <- data$data[[group]]
# datasex <- data$data$sex
data <- pins::pin_read(board, data_pin) %>%
  mutate(
    imd_quintile = dplyr::case_when(imd_quintile %in% "1" ~ "Quintile 1 - Most Deprived", TRUE ~ imd_quintile),
    imd_quintile = dplyr::case_when(imd_quintile %in% "2" ~ "Quintile 2", TRUE ~ imd_quintile),
    imd_quintile = dplyr::case_when(imd_quintile %in% "3" ~ "Quintile 3", TRUE ~ imd_quintile),
    imd_quintile = dplyr::case_when(imd_quintile %in% "4" ~ "Quintile 4", TRUE ~ imd_quintile),
    imd_quintile = dplyr::case_when(imd_quintile %in% "5" ~ "Quintile 5 - Least Deprived", TRUE ~ imd_quintile)
  ) %>%
  select(-lsoa_code)

if(group == "caring"){
data <- data %>%
  dplyr::mutate(caring = dplyr::case_when(caring == "Yes" ~ "Young carer",
                            caring == "No" ~ "Non-carer",
                            caring == "Prefer not to say" ~ "Prefer not to say",
                            TRUE ~ NA_character_))
...
if(group == "bullied"){
  data <- data %>%
  dplyr::mutate(bullied = dplyr::case_when(bullied == "Yes" ~ "Bullied",
                            bullied == "No" ~ "Not bullied",
                            bullied == "Prefer not to say" ~ "Prefer not to say",
                            TRUE ~ NA_character_))
...
if(group == "selfharm_ever"){
  data <- data %>%
  dplyr::mutate(selfharm_ever = dplyr::case_when(selfharm_ever == "Yes" ~ "Self-harmed",
                            selfharm_ever == "No" ~ "Not self-harmed",
                            selfharm_ever == "Prefer not to say" ~ "Prefer not to say",
                            TRUE ~ NA_character_))
...
responses_per_district <- data %>%
  ...
  dplyr::filter(school %in% school_vec) %>%
...
get_stats <- function(data,
  ...
  data <- data %>%
    dplyr::mutate(life_satisfied = dplyr::recode(life_satisfied, "0"="low","1"="low","2"="low","3"="low","4"="low","5"="medium","6"="medium","7"="high","8"="high","9"="very high","10"="very high"),
                  life_worthwhile = dplyr::recode(life_worthwhile, "0"="low","1"="low","2"="low","3"="low","4"="low","5"="medium","6"="medium","7"="high","8"="high","9"="very high","10"="very high"),
                  life_happyyesterday = dplyr::recode(life_happyyesterday, "0"="low","1"="low","2"="low","3"="low","4"="low","5"="medium","6"="medium","7"="high","8"="high","9"="very high","10"="very high"),
                  life_satisfied_before_covid = dplyr::recode(life_satisfied_before_covid, "0"="low","1"="low","2"="low","3"="low","4"="low","5"="medium","6"="medium","7"="high","8"="high","9"="very high","10"="very high"),
                  alcohol_perday = dplyr::recode(alcohol_perday, "0"="none","1"="1-2","2"="1-2","3"="3-4","4"="3-4","5"="5-6","6"="5-6","7"="7-9","8"="7-9","9"="7-9","10"="10+"),
                  pa_60 = dplyr::recode(pa_60, "0"="none","1"="1-3","2"="1-3","3"="1-3","4"="4-5","5"="4-5","6"="6-7","7"="6-7"),
                  pa_30 = dplyr::recode(pa_30, "0"="none","1"="1-3","2"="1-3","3"="1-3","4"="4-5","5"="4-5","6"="6-7","7"="6-7"))
  ...
  if (!is.na(sch[1])) {
    ...
    for (g in 1:length(group)) {
      ...
      data_l_d[[paste0("data_school_by", group[g])]] <- data %>% 
        ...
        # temporarily bind full data for complete
        dplyr::bind_rows(dplyr::select(stats, school, question, response) %>% dplyr::filter(school == "All Schools")) %>% 
        tidyr::complete(expand(., school, .data[[group[g]]], 
                        nesting(question, response))) %>% 
        dplyr::group_by_all() %>%
        dplyr::filter(!is.na(response)) %>%
        dplyr::summarise(count = dplyr::n()) %>%
      ...
      if(!is.na(group_of_interest)) {
        ...
        for(group in 1:length(c("All Responses", group_of_interest))) {
          ...
          df <- grp_df %>% 
            ...
            tidyr::drop_na(reworded) %>% 
            dplyr::arrange(desc(count))
          
          if (binary) {
            temp <- paste0("Out of responses from ", group_name, ", ", 
                           glue::glue_collapse(glue::glue("{df$value} selected '{df$question_text.x}'"), ", ", last = ", and "))
          } else{
            temp <- paste0("The number of ", group_name, " who stated '", df$response[1], "' was ",
                           glue::glue_collapse(glue::glue("{df$value} for '{df$question_text.x}'"), ", ", last = ", and "))
          }
...
mean_responses_plot <- function(df = data, vars = chk_var, custom_grp = group){
  ...
  df_year <- df %>%
    dplyr::select(breakdown = schyear, vars) %>%
    dplyr::mutate(across(vars, ~as.numeric(ifelse(.x == "Yes", 1, 0)))) %>%
    dplyr::mutate(total = rowSums(across(vars), na.rm = T)) %>%
    dplyr::group_by(breakdown) %>%
    PHEindicatormethods::phe_mean(total) %>%
    dplyr::ungroup()
  
  df_sex <- df %>%
    dplyr::select(breakdown = sex, vars) %>%
    dplyr::mutate(across(vars, ~as.numeric(ifelse(.x == "Yes", 1, 0)))) %>%
    dplyr::mutate(total = rowSums(across(vars), na.rm = T)) %>%
    dplyr::group_by(breakdown) %>%
    PHEindicatormethods::phe_mean(total) %>%
  ...
  df_custom <- df %>%
    dplyr::select(breakdown := !!ensym(custom_grp), vars) %>%
    dplyr::mutate(across(chk_var, ~as.numeric(ifelse(.x == "Yes", 1, 0)))) %>%
    dplyr::mutate(total = rowSums(across(vars), na.rm = T)) %>%
    dplyr::group_by(breakdown) %>%
    PHEindicatormethods::phe_mean(total) %>%
    dplyr::ungroup() %>%
    dplyr::filter(breakdown != "Prefer not to say")
  
  df <- dplyr::bind_rows(df_year, df_sex, df_custom) %>% dplyr::filter(!is.na(breakdown))
  ...
  df %>%
    ...
    echarts4r::e_y_axis(name = "Mean", nameLocation = "middle", nameGap = 35) %>% 
    echarts4r::e_x_axis(axisLabel = list(interval = 0, rotate = 90)) %>%
    echarts4r::e_title(paste("Mean number of", tolower(plot_title))) %>%
    echarts4r::e_image_g(right = 80, top = 5, z = -999, style = list(opacity = 0.5, width = 120,
                                                          image = "https://www.hertshealthevidence.org/images/young-peoples-health-and-wellbeing-survey-logo-png-Cropped-448x190.png")) %>%
    echarts4r::e_toolbox_feature(feature = c("dataZoom", "saveAsImage")) %>%
    echarts4r::e_grid(bottom = 150)
...
create_tbl <- function(stats_diff, multi_response = F, include_school = F) {
  ...
  if (multi_response == F) {
    ...
    data <- stats_diff %>% 
      dplyr::mutate(difference = case_when(lowercl.x > uppercl.y ~ "higher",
    ...
    if(length(unique(data$response)) <= 2) {
      ...
      data1 <- data %>% 
        ...
        dplyr::select(1, question=question_text.x, response, count, percentage, `lower CI`, `upper CI`) %>% 
        dplyr::distinct() %>% 
        dplyr::mutate(`upper CI` = as.character(`upper CI`),
               `lower CI` = as.character(`lower CI`))
...
create_bullets_herts <- function(df, grouped_by_question = F) {
  ...
  df <- df %>%
    dplyr::filter(question %in% keep_these) %>%
    dplyr::left_join(select(q_coded, question_coded, question_theme), by = c("question" = "question_coded")) %>%
    dplyr::rename(Theme = question_theme)
  ...
  if(nrow(df) == 0){
    ...
    df <- dplyr::rename(df, "Differences found" = sentences, "Breakdown" = breakdown.x,
    ...
    #make the table
    l[["Table"]] <- reactable::reactable(dplyr::select(df, Breakdown, Theme, Question, `Differences found`),
                              groupBy = groupedc, highlight = T, striped = T,
                              pagination = F,
                              columns = list(`Differences found` = reactable::colDef(minWidth = 200, html = T)  #minimum width 200
                              ))
...
ls1 <- ifelse(!is.na(cat_of_interest), paste0("This statistic was ", 
                                              dplyr::filter(stats_all, question == 'pa_60' & response == "6-7") %>% 
                                                .$value, " for all responses."), "")
...
chk_diff <- dplyr::filter(stats_diffs, question %in% chk_var) %>%
  dplyr::mutate(response = factor(response, levels = c("none", "1-3", "4-5", "6-7")))%>%
  dplyr::mutate(response = factor(response, levels = c("none", "1-3", "4-5", "6-7"))) %>%
  dplyr::mutate(response = factor(response, levels = c("none", "1-3", "4-5", "6-7")))
