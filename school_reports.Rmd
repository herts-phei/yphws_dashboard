---
title: '<center>![](../www/logo2.png){width=250px}</center>'
pagetitle: '`r paste("YPHWS School Report", current_year)`'
author: "Public Health Evidence & Intelligence, Hertfordshire County Council"
date: '`r paste("YPHWS", current_year)`'
logo: "../www/logo2.png"
output:
  rmdformats::robobook:
    css: "../www/robobook.css"
  self_contained: true
thumbnails: false
lightbox: false
gallery: false
...
# 
source("~/projects/yphws_school_report/R/0_report_functions.R")
source("~/projects/yphws_school_report/R/1_postcode_update.R")
source("~/projects/yphws_school_report/R/2_data_cleaning.R")
source("~/projects/yphws_school_report/R/3_generate_school.R")
# 
...
create_report_chunk <- function(df, 
  ...
  for (i in vars) {
    ...
    n_responses <- stats %>% 
      filter(question %in% chk_var, school == sch, breakdown == "All Responses")
    ...
    if (simple) {
      ...
      livcon_plot_list[[i]] <- create_basic_plot(df = chk_stats,
                                               diff_df = chk_diff_sch, 
                                               sex = T, 
                                               sch = sch, 
                                               plot_title = plot_title, 
                                               year = current_year, 
                                               textangle = plot_angle)
      
      livcon_plot2_list[[i]] <- create_basic_plot(df = chk_stats, diff_df = chk_diff_sch, sex = F, sch = sch, plot_title = plot_title, year = current_year, textangle = plot_angle)
      
      livcon_table_list[[i]] <- create_tbl(chk_diff_sch, q_coded = q_coded)
      ...
      if (!all(is.na(vars_df$response_filter_top[vars_df$question_coded_gen == i]))) {
        ...
        chk_var_filtered <- chk_stats %>%
          filter(response == response_interest, breakdown == "All Responses", school == sch) %>%
          arrange(desc(count)) %>%
          slice(1:filter_to) %>%
          pull(question)
      ...
      livcon_plot_list[[i]] <- create_multicat_plot_herts(df = chk_stats, diff_df = chk_diff_sch, sch = sch, q_coded = q_coded, title = plot_title, facet = "breakdown")$result
      
      livcon_plot2_list[[i]] <- glue::glue(" ")
      
      livcon_table_list[[i]] <- create_tbl(chk_diff_sch, q_coded = q_coded, multi_response = T)
...
> Please let us know how useful you find this report and any other feedback regarding the survey by filling out <a href="https://surveys.hertfordshire.gov.uk/s/85GUNP?source=`r survey_code`" target="_blank"> this short 5 question survey</a>. Your feedback is greatly appreciated.

This report summarises the results of the `r filter(stats, school==sch) %>% select(denominator) %>% max()` pupils from `r sch` who responded to the `r current_year` Young People's Health & Wellbeing Survey (YPHWS).
...
A .csv of summarised results will be provided to schools so they can dig more deeply into the data if they wish. 
...
<font size="3.5"> This section is an overview of the various health topics covered within the report for `r sch`. <br>
`r low_denom_warning`

**Mental health and wellbeing**<br>

- **`r paste0(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'life_satisfied' & response == "low") %>% pull(value))`** of respondents rated their life satisfaction as low. `r mh_1`

- **`r paste0(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'hopeful_future' & response == "Never") %>% pull(value))`** of respondents stated that they never feel hopeful about their future. `r mh_7`


- **`r paste0(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'worry_money' & response == "Yes") %>% pull(value))`** of respondents stated they worry about money problems (including family finances). `r mh_3`

- The **top 5 issues** respondents were worried about were: `r mh_4`

- The **top 5 ways of coping** with a problem that respondents were worried about were: `r mh_5`

- **`r sum(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'mental_howaccess' & response != 'Yes') %>% .$value)`** of respondents answered "Not sure" or "No" when asked if they knew how to access support and services for mental health. **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'mental_howaccess' & response == 'No') %>% .$value`** answered "No". `r mh_6`

**Lifestyle** <br>

-   Out of all responses, **`r paste0(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'pa_60' & response == "6 to 7") %>% .$value)`** had done a total of 60 minutes or more of physical activity a day in the past week. `r ls_1` When asked how many days respondents have done a total of 60 minutes physical activity, pupils in `r sch` commonly responded with **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question =='pa_60') %>% filter(count == max(count)) %>% .$response`** day(s).

-   **`r sum(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'vaping' & response != 'I have never vaped') %>% .$value)`** of respondents reported having ever vaped while **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'vaping' & response == "I vape regularly (once a week or more)") %>% .$value`** reported vaping once a week or more (`r ls_3`). 

-   **`r sum(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'alcohol_ever' & response != 'Never') %>% .$value)`** of respondents reported having had an alcoholic drink in the past 3 months. **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'alcohol_ever' & response == '4 or more times a week') %>% .$value`** reported drinking 4 or more times a week (`r ls_4`). 

-   **`r sum(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'drug_ever' & response != 'I have never taken drugs') %>% .$value)`** of respondents reported having ever taken drugs while **`r filter(stats, year == as.character(current_year), breakdown == 'All Responses' & question == 'drug_ever' & response == 'I take drugs regularly (once a week or more)') %>% .$value`** reported taking drugs once a week or more (`r ls_5`). 

**Safety** <br>

-   Regarding safety, **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'safety_day' & response == 'Unsafe') %>% .$value`** of respondents felt unsafe going out during the day, **`r filter(stats, year == as.character(current_year), breakdown == 'All Responses' & question == 'safety_dark' & response == 'Unsafe') %>% .$value`** of respondents felt unsafe going out after dark, **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'safety_school' & response == 'Unsafe') %>% .$value`** of respondents felt unsafe at school, and **`r filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'safety_journey' & response == 'Unsafe') %>% .$value`** of respondents felt unsafe going to and from school.

**Bullying**

- **`r paste0(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'bullied' & response == "Yes") %>% pull(value))`** of respondents stated that they have been bullied in their life. `r bully_1`

- **`r paste0(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'bullied_currently' & response == "Yes") %>% pull(value))`** of respondents stated that they are currently being bullied. `r bully_2`

**School support**<br>

-   **`r sum(filter(stats, year == as.character(current_year),  breakdown == 'All Responses' & question == 'schoolsupp_academic' & response == "Yes") %>% .$value)`** of respondents felt that their school is supportive of their academic development. `r ss_1`

-   **`r sum(filter(stats, year == as.character(current_year), breakdown == 'All Responses' & question == 'schoolsupp_wellbeing' & response == "Yes") %>% .$value)`** of respondents felt that their school is supportive of their emotional health and wellbeing. `r ss_2`

</font>

```{r chunk_processing, include=FALSE}
...
```{r create_demographics, include=FALSE}
...
```{r demographics_chks, include=FALSE}
...
```{r create_living_conditions, include=FALSE}
...
```{r living_conditions_chks, include=FALSE}
...
```{r create_lifestyle, include=FALSE}
...
```{r lifestyle_chks, include=FALSE}
...
```{r create_smokevape, include=FALSE}
...
```{r smokevape_chks, include=FALSE}
...
```{r create_alccons, include=FALSE}
...
```{r create_druguse, include=FALSE}
...
```{r create_sexualhlth, include=FALSE}
...
```{r sexualhlth_chks, include=FALSE}
...
```{r create_mntlhlth, include=FALSE}
...
```{r create_sustainenviron, include=FALSE}
...
```{r sustainenviron_chks, include=FALSE}
